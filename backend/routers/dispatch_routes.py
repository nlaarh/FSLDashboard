...
router = APIRouter()
log = logging.getLogger('dispatch_routes')
...
@router.get("/api/_legacy/insights/reassignment-detail")
...
@router.get("/api/_legacy/insights/human-intervention")
...
def _build_zone_satisfaction(all_garages, matrix):
    ...
    try:
        ...
        log.warning("Unable to load ops garages for zone mapping: %s", e)
