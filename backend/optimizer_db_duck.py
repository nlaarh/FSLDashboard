"""DuckDB backend for optimizer store — schema, connection, query helpers.

Loaded by optimizer_db.py when OPT_DB_BACKEND=duckdb (default).
"""
...
log = logging.getLogger('optimizer_db_duck')
...
# DuckDB supports both INSERT OR REPLACE and ON CONFLICT — we use OR REPLACE
# for brevity. Mirrors the API in optimizer_db_pg.py for backend swap.
...
def _build_replace_sql(table: str, cols: tuple) -> str:
    col_list = ', '.join(cols)
...
def upsert_run(row: dict) -> None:
    sql = _build_replace_sql('opt_runs', _RUN_COLS)
    with get_conn() as conn:
...
def bulk_upsert_decisions(rows: list[dict]) -> None:
    if not rows:
        return
    sql = _build_replace_sql('opt_sa_decisions', _DECISION_COLS)
    with get_conn() as conn:
...
def bulk_upsert_verdicts(rows: list[dict]) -> None:
    if not rows:
        return
    sql = _build_replace_sql('opt_driver_verdicts', _VERDICT_COLS)
    with get_conn() as conn:
