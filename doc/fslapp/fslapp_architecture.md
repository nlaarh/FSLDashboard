...
- **Fleet ATA**: `CreatedDate → ActualStartTime` (real driver arrival from FSL mobile app)
- **Towbook ATA**: `CreatedDate → ServiceAppointmentHistory 'On Location'` (real arrival from Towbook integration)
  - NEVER use `ActualStartTime` for Towbook — it's a midnight bulk-update, not real arrival
  - Backend helper: `sf_client.get_towbook_on_location(sa_ids)` batch-fetches real timestamps
  - If no 'On Location' history exists for a SA, that SA is excluded from ATA metrics
- Guardrail: `0 < ATA < 480 min` for all metric calculations
...
- `ERS_Dispatch_Method__c` on SA — **ALSO a formula field** (confirmed via sf_describe), can't GROUP BY
