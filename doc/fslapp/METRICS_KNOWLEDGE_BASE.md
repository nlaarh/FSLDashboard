...
| `SchedStartTime` | ServiceAppointment | DateTime (standard) | The scheduler's **predicted arrival time** — when the driver is expected to arrive on site and begin service. Set by FSL optimization engine or manually by dispatcher. Updated each time the scheduler re-optimizes. NOT the assignment time (that's AssignedResource.CreatedDate). Correct decomposition: Queue = CreatedDate → AssignedResource.CreatedDate, Est. Travel = AssignedResource.CreatedDate → SchedStartTime, Prediction Error = SchedStartTime → ActualStartTime. |
...
| `ERS_Dispatch_Method__c` | ServiceAppointment | Formula (custom) | `'Field Services'` = Fleet (internal). `'Towbook'` = Contractor (external). Derived via formula (from facility account dispatch method, which Mulesoft controls indirectly). Determines which ATA calculation to use. **IS a formula field** (confirmed via sf_describe) — can't GROUP BY in SOQL. `ERS_Facility_Dispatch_Method__c` on WorkOrder is ALSO a formula. |
...
```
Metrics (scorer, scorecard, performance, ops):  0 < ATA < 480  (8 hours)
Display-only (sa_lookup, open wait times):      0 < ATA < 1440 (24 hours)
```

The 480-min guardrail keeps historical aggregates clean. The 1440-min cap is only for single-SA display and open-call wait times where longer values may be legitimate.
...
- Towbook ATA = `OnLocation − CreatedDate` (from ServiceAppointmentHistory)
- Only SAs where `0 < ATA < 480` minutes AND `0 < ERS_PTA__c < 999` are evaluated
- Delta = `actual_arrival − (CreatedDate + ERS_PTA__c minutes)` — negative = early, positive = late
- **Default if missing:** `null` → `—`
...
```
Bucket 1: ATA < 45 min       (under_45)
Bucket 2: 45 ≤ ATA < 90 min  (b45_90)
Bucket 3: 90 ≤ ATA < 120 min (b90_120)
Bucket 4: ATA ≥ 120 min      (over_120)
```

Code: main.py `_compute_performance()` lines 2603-2617.
...
|-----------|--------|--------|---------------|---------|
| SLA Hit Rate | 30% | 100% | Yes | `count(ATA ≤ 45) ÷ count(valid ATAs)` — guardrail: `0 < ATA < 1440` |
| Completion Rate | 15% | 95% | Yes | `count(Completed) ÷ count(all SAs)` |
| Satisfaction | 15% | 82% | Yes | `count('totally satisfied'.lower()) ÷ count(surveys)` |
| Median Response | 10% | 45 min | No | `median(all ATAs)` — guardrail: `0 < ATA < 1440` |
| PTA Accuracy | 10% | 90% | Yes | `count(ATA ≤ ERS_PTA__c) ÷ count(evaluated)` — guardrail: `0 < ATA < 480` (stricter) |
| Could Not Wait | 10% | 3% | No | `count(CNW cancellations) ÷ count(all SAs)` |
| Dispatch Speed | 5% | 5 min | No | `median(SchedStartTime − CreatedDate)` — NOTE: this includes queue wait + estimated travel, not just queue time. Pure queue = AssignedResource.CreatedDate − CreatedDate |
| Decline Rate | 5% | 2% | No | `count(declines) ÷ count(all SAs)` |

**Note:** Scorer uses TWO different ATA guardrails: 1440 min (24h) for SLA/response metrics, 480 min (8h) for PTA accuracy. This is intentional — SLA counts any legitimate response, PTA accuracy only evaluates realistic ETAs.
...
For each completed SA:
  Dispatch Queue     = CreatedDate → AssignedResource.CreatedDate (time waiting for assignment)
  Estimated Travel   = AssignedResource.CreatedDate → SchedStartTime (scheduler's predicted travel)
  Prediction Error   = SchedStartTime → driver arrival (positive = late, negative = early)
  On-Site Service    = driver arrival → ActualEndTime

  Fleet: driver arrival = ActualStartTime
  Towbook: driver arrival = On Location from SAHistory
```
